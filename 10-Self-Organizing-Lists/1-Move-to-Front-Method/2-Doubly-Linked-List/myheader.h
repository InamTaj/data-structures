...
#include<conio.h>
using namespace std;
//Objective: Implement a Self organizing Doubly Linked List
...
{
	NODE<mytype> *temp = head;	//created a new node pointer to copy head
	if (head->data==element)	//if head has element
	{
		//then do nothing
	}
	else if(tail->data==element)
	{
		temp=tail;			//make copy of tail
		tail=tail->prev;	//move tail to previous node
		tail->next=0;		//nullify tail's next
		temp->next=head;	//put head in temp's next
		head->prev=temp;	//store address of temp in head's previous
		temp->prev=0;		//nullify temp's previous
		head=temp;			//move head 1 step back
	}
	else //Other wise if data is somewhere in List then
	{
		...
		if (temp->data==element)	//if data is found then
		{
			temp->prev->next=temp->next;	//store address of temp's next node in next part of temp's previous node
			temp->next->prev=temp->prev;	//store address of temp's previous node in previous part of temp's next node
			temp->next=head;		//store head in temp's next
			head->prev=temp;		//store address of temp in head's previous
			temp->prev=0;			//nullify temp's previous
