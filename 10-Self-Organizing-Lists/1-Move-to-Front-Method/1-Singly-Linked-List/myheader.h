...
NODE<mytype>* SingleLinkList<mytype>::find_node(mytype element)
{
	...
	else	//Other wise if data is somewhere in List then
	{
		...
		{
			NODE<mytype> *pred = head;	//created a copy of head
			while (pred->next!=temp)	//until pred reaches the node which has required data
			{
				pred = pred->next;		//move pred to next node
			}
			pred->next=temp->next;		//store address of temp's next node in pred's next
			temp->next=head;			//moved temp node to start of list
			head=temp;					//pointed head to temp
		}
	}
	return temp;
