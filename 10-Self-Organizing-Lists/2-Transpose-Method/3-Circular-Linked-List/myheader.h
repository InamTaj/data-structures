...
#include<conio.h>
using namespace std;
//Objective: Implement a Circular Linked List
//--------------------
...
bool CircularLinkList<mytype>::search_item(mytype element)
{
	if(!is_Empty())
	{
		...
		else
		{
			NODE<mytype> *temp = tail->next;		//created a new node pointer to copy head
			mytype var;
			while(temp->next!=tail && temp->data!=element)//until Queue ends or data is found (& is used because search STOPS if data is found in a node)
			{
				temp=temp->next;	//move temp node to next node
			}
