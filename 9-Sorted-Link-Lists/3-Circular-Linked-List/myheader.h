//Objective: Implement a Sorted Circular Linked List
...
public:
	...
	void add2CLLhead(mytype element);	//Function to add node to head
	void add2CLLtail(mytype element);	//Function to add note to tail
...
template <class mytype>
void CircularLinkList<mytype>::add_item(mytype element)
{
	if(is_Empty())
	{
		add2CLLhead(element);	//If list is empty, then add to head
	}
	else
	{
		NODE<mytype> *temp = new NODE<mytype>(element, 0);	//Created a new node
