//Objective: Implement a Sorted Doubly Linked List
...
public:
	...
	void add2DLLhead(mytype element);	//Function to add node to head 
	void add2DLLtail(mytype element);	//Function to add note to tail
...
template <class mytype>
void DoublyLinkedList<mytype>::add_item(mytype element)
{
	if(is_Empty())
	{
		add2DLLhead(element);	//If list is empty, then add to head
	}
	else
	{
		...
		{
			add2DLLtail(element);	//add new node to tail
		}
		else if(temp->data<=head->data)	//If new node's data is smaller than head's data, then
		{
			add2DLLhead(element);	//add new node to head
		}
		else	//otherwise
		{
